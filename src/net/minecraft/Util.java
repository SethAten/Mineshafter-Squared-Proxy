...
public class Util {
		...
		for (Map.Entry<String, Object> localEntry : paramMap.entrySet()) {
			...
			try {
				localStringBuilder.append(URLEncoder.encode(
						(String) localEntry.getKey(), "UTF-8"));
			...
			if (localEntry.getValue() != null) {
				localStringBuilder.append('=');
				try {
					localStringBuilder.append(URLEncoder.encode(localEntry
	...
	public static String executePost(String targetURL, String urlParameters) {
		...
		try {
			...
			c.setRequestProperty("Host", u.getHost()); // does this get the
														// original url host, or
														// the redirect one?
			...
			BufferedOutputStream tos = new BufferedOutputStream(
					c.getOutputStream());
			tos.write(params);
			tos.flush();
			tos.close();
